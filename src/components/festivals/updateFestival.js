...
export const UpdateFestival = ({row,setTrig}) => {
    ...
    const handleUpdate = async () => {
        ...
        const [response, response1,response2,response3] = await Promise.all([
            ...
            await requestToBack('PUT',{prix_table_espace:festivalPost.prix_table_espace_1,prix_surface_espace:festivalPost.prix_surface_espace_1,nb_table_espace:festivalPost.nb_table_espace_1},`/espace/${row.espaces[0].id_espace}`,authHeader()),
            await requestToBack('PUT',{prix_table_espace:festivalPost.prix_table_espace_2,prix_surface_espace:festivalPost.prix_surface_espace_2,nb_table_espace:festivalPost.nb_table_espace_2},`/espace/${row.espaces[1].id_espace}`,authHeader()),
            await requestToBack('PUT',{prix_table_espace:festivalPost.prix_table_espace_3,prix_surface_espace:festivalPost.prix_surface_espace_3,nb_table_espace:festivalPost.nb_table_espace_3},`/espace/${row.espaces[2].id_espace}`,authHeader())
    ...
    return (
        <div>
            ...
            <UpdateFestivalModal titre="Editer festival" row={festivalPost} setRow={setFestivalPost} onClose={handleCloseEdit} onUpdate={handleUpdate} open={openEdit}/>
        </div>
